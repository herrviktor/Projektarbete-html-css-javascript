...
document.querySelectorAll('.product-button').forEach(button => {
    button.addEventListener('click', function() {
      ...
      const price = this.parentElement.querySelector('.product-price').textContent;
      let shoppingData = JSON.parse(localStorage.getItem('shoppingData')) || [];
      shoppingData.push({ title, price });
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    shoppingData.forEach(item => {
      ...
      listItem.textContent = `${item.title} - ${price} kr`;
      shoppingList.appendChild(listItem);
    });
    const totalSum = shoppingData.reduce((sum, item) => sum + parseFloat(item.price.replace(/\D/g, '')), 0);
    const shoppingTotalElement = document.getElementById('shopping-total');
    shoppingTotalElement.textContent = `Totalbelopp: ${totalSum} kr`;
});